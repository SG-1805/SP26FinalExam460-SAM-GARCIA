...
|---|---|
| Extrance/spawn node | This is nessecary to compute the cheapest cost from the starting locationto each relic  |
| Relic chamber nodes | this is needed to compute the cheapest costs from each relic to the remaining relics and ti the exit |
...
| What the values represent |Shortest path fuel costs frok the source node to the destination node |
